...
public class DragSortListView extends ListView {
    ...
    protected void dispatchDraw(@NonNull Canvas canvas) {
        ...
        if (mFloatView != null) {
            ...
            canvas.save();
            // Log.d(tag, "clip rect bounds: " + canvas.getClipBounds());
    ...
    private void printPosData() {
        Log.d(TAG, "mSrcPos=" + mSrcPos + " mFirstExpPos=" + mFirstExpPos + " mSecondExpPos="
                + mSecondExpPos);
    }
    ...
    private boolean updatePositions() {
        ...
        // Log.d(tag, "float mid="+mFloatViewMid);
        ...
        if (mAnimate) {
            ...
            // Log.d(tag, "edgeTop="+edgeTop+" edgeBot="+edgeBottom);
    ...
    private void adjustOnReorder() {
        ...
        // Log.d(tag, "first="+firstPos+" src="+mSrcPos);
        if (mSrcPos < firstPos) {
            ...
            // Log.d(tag, "top="+top+" fvh="+mFloatViewHeight);
     ...
     * wrapped item.
     */
    private void adjustItem(int position, @NonNull View v, boolean invalidChildHeight) {
    ...
    private int getChildHeight(int position) {
        ...
        if (v != null) {
            ...
            if (childHeight != -1) {
                // Log.d(tag, "found child height in cache!");
                return childHeight;
            }
    ...
    private int getChildHeight(int position, @NonNull View item, boolean invalidChildHeight) {
        if (position == mSrcPos) {
            return 0;
        }
    ...
    private int calcItemHeight(int position, @NonNull View item, boolean invalidChildHeight) {
    ...
    private int adjustScroll(int movePos, @NonNull View moveItem, int oldFirstExpPos, int oldSecondExpPos) {
        int adjust = 0;
    ...
    protected boolean onDragTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (ev.getAction() & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_CANCEL:
                ...
                break;
            case MotionEvent.ACTION_UP:
                // Log.d(tag, "calling stopDrag from onDragTouchEvent");
                if (mDragState == DRAGGING) {
                    stopDrag(false);
                }
                doActionUpOrCancel();
    ...
    private void doDragFloatView(int movePos, @NonNull View moveItem, boolean forceInvalidate) {
        ...
        if (updated) {
            ...
            // Log.d(tag, "  adjust scroll="+scroll);
    ...
    private void updateFloatView() {
        ...
        // Log.d(tag, "dragView top=" + (y - mDragDeltaY));
        // Log.d(tag, "limit=" + limit);
        // Log.d(tag, "mDragDeltaY=" + mDragDeltaY);
    ...
    private class DragScroller implements Runnable {
        ...
        public void run() {
            ...
            if (scrollDir == UP) {
                View v = getChildAt(0);
                // Log.d(tag, "vtop="+v.getTop()+" padtop="+padTop);
                if (v == null) {
                    mScrolling = false;
                    return;
                } else {
            ...
            mPrevTime = mCurrTime;
            // Log.d(tag, "  updated prevTime="+mPrevTime);
        ...
        public DragSortTracker() {
            ...
            if (!mFile.exists()) {
                try {
                    ...
                    Log.e(TAG, "Could not create dslv_state.txt",e);
                }
            }
