...
typealias Task = List<TToken>

private inline fun <reified T> Task.getFirstToken(): T? {
    this.filterIsInstance<T>().forEach {
        return it
    }
    return null
}

private inline fun <reified T> Task.getAllTokens(): List<T> {
    return this.filterIsInstance<T>()
}

private inline fun <reified T : TToken> Task.upsertToken(newToken: T): Task {

    return if (getFirstToken<T>() == null) {
        this.plus(newToken)
    } else {
        this.mapIndexed { idx, item ->
            if (item is T) newToken else item
        }
    }
    ...
    fun Task.text(): String = this.joinToString(" ") { it.text }
...
//        }
//
fun Task.priority(): Priority = getFirstToken<PriorityToken>()?.value ?: Priority.NONE

fun Task.setPriority(prio: Priority): Task {
    return when {
        prio == Priority.NONE -> this.filter { it is PriorityToken }
        this.any { it is PriorityToken } -> upsertToken(PriorityToken(prio.fileFormat))
        else -> listOf(PriorityToken(prio.fileFormat)).plus(this)
    }
}

fun Task.recurrencePattern(): String?  = this.getFirstToken<RecurrenceToken>()?.valueStr
fun Task.tags(): SortedSet<String>   = this.getAllTokens<TagToken>().map {it.value}.toSortedSet()
fun Task.lists(): SortedSet<String>   = this.getAllTokens<ListToken>().map {it.value}.toSortedSet()
fun Task.links(): Set<String>   = this.getAllTokens<LinkToken>().map {it.value}.toSet()
fun Task.phoneNumbers(): Set<String>   = this.getAllTokens<PhoneToken>().map {it.value}.toSet()
fun Task.mailAddresses(): Set<String>   = this.getAllTokens<MailToken>().map {it.value}.toSet()
fun Task.uuid(): String?   = this.getFirstToken<UUIDToken>()?.valueStr
fun Task.isCompleted() : Boolean = this.getFirstToken<CompletedToken>() != null
fun Task.alphaParts(): String = this.showParts { it.isAlpha() }

    fun Task.removeTag(tag: String) : Task = this.filter {
            ((it is TagToken) && it.value == tag)
        }

fun Task.removeList(tag: String) : Task = this.filter {
    ((it is ListToken) && it.value == tag)
}

    fun Task.markComplete(dateStr: String): Pair<Task, Task?> {
...
//    }
//
    fun Task.inFileFormat(useUUIDs: Boolean) : String {
        return if (useUUIDs && text().isNotBlank() && uuid() == null) {
            "${text()} uuid:${UUID.randomUUID()}"
        } else {
            text()
        }
    }
...
//
    ...
    fun Task.showParts(filter: (TToken) -> Boolean): String {
